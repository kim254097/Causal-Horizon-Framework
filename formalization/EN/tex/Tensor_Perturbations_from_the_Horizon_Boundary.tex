...
\title{Tensor Perturbations from the Horizon Boundary: Gravitational Wave Spectrum and Consistency Relations}
\author{Kevin Mu\~noz}
\date{}
...
\section{Tensor Degrees of Freedom on the Apparent Horizon}

\subsection{Identifying the boundary tensor modes}
...
Each polarization mode $h_k^\lambda(t)$ with comoving wavenumber $k$ is identified with the angular mode $l_k(t) = k/(a(t)H(t))$ on the horizon 2-sphere, exactly as in the scalar case (Step~1, eq.~(2)). Freeze-out occurs when $l_k = 1$, i.e., $k = a(t_k)H$, which is the standard horizon-crossing condition.

\section{The Boundary Tensor Action}

\subsection{Dimensional reduction from the bulk}
...
\end{equation}
where primes denote $d/d\eta$.

\subsection{Canonical variable}
...
\end{equation}
This is the \textbf{Mukhanov--Sasaki equation for tensor modes}, with pump field $z_T = a$.

\subsection{Comparison with the scalar case}

In Step~1, the scalar canonical variable was $u_k = z_A\psi_k$ with $z_A = a/\sqrt\varepsilon$. For tensor modes, the pump field is simply $z_T = a$. Specifically:
\begin{equation}
...
\section{Bunch--Davies Vacuum and Tensor Freeze-Out}
...
\subsection{Exact solution and freeze-out amplitude}
...
\end{equation}
Freeze-out occurs at $k\eta_k = -1$. For de~Sitter ($\nu_T = 3/2$):
\begin{equation}
...
\end{equation}
This is exactly the standard slow-roll inflation result for the primordial gravitational wave amplitude.

\paragraph{Tensor spectral index.}
...
\end{equation}
The tensor spectrum is slightly red-tilted, with the tilt determined entirely by $\varepsilon = (1/2)d\ln S/d\ln a$.

\section{Tensor-to-Scalar Ratio}

Combining the tensor spectrum (15) with the scalar power spectrum of the comoving curvature perturbation $\mathcal{R}$:
\begin{equation}
...
\paragraph{Physical interpretation within the CHF.} The factor $\varepsilon^{-2}$ that suppresses the scalar spectrum relative to tensor has a twofold origin: (1) the boundary scalar field $\psi$ is suppressed by $\varepsilon$ relative to $\mathcal{R}$ (from $\psi \approx \varepsilon\mathcal{R}$, Step~2), contributing a factor $\varepsilon^2$ to $P_\psi/P_\mathcal{R}$; (2) the tensor boundary field carries no such suppression (shape perturbation, no $\varepsilon$ in $z_T$). Together: $P_h \sim H^2/M_{\rm Pl}^2$, $P_\mathcal{R} \sim H^2/(M_{\rm Pl}^2\varepsilon)$, giving $r = P_h/P_\mathcal{R} \propto \varepsilon$. The constant is fixed to $16$ by the GR normalization.

\section{Standard Consistency Relations}

\subsection{The Lyth--Liddle relation}
...
\subsection{Additional CHF consistency relation}
...
\end{equation}
This collapses the two-dimensional freedom of standard inflation to a single line in the $(n_s, r)$ plane.

\paragraph{Standard inflation comparison.} In standard single-field inflation with two slow-roll parameters $\varepsilon$ and $\eta'$:
\begin{equation}
n_s - 1 = -2\varepsilon - \eta', \qquad n_T = -2\varepsilon \tag{23}
\end{equation}
so $n_T - (n_s - 1) = \eta' \neq 0$ in general. The CHF predicts $\eta' = 0$ (or equivalently, $n_T = n_s - 1$), which is violated for most inflationary potentials.

\section{Resolution of the Step~5 Tension}

Step~5 identified a tension: the minimal CHF ($\beta_S = 0$) with the assumption $r = 16\varepsilon$ predicts $r = 8(1-n_s) \approx 0.281$ for $n_s = 0.9649$, exceeding the Planck 2018 bound $r < 0.056$. The present derivation confirms that $r = 16\varepsilon$ does hold within the CHF. The tension is therefore real and cannot be resolved by appealing to a different tensor spectrum.

\subsection{Non-minimal scalar field ($\beta_S \neq 0$)}
...
\end{equation}
A slightly tachyonic horizon scalar field ($m_{\rm eff}^2 = \beta_S H^2 < 0$) is required. A small negative $\beta_S$ is physically plausible and does not destabilize the mode equation, since the instability scale is $\sim m_{\rm eff} \sim \sqrt{|\beta_S|}H \ll M_{\rm Pl}$.

\subsection{Interpretation of the tension as falsification}
...
\end{equation}
For $n_s = 0.965$: $\varepsilon > 0.004$, giving $r = 16\varepsilon > 0.064$ --- a contradiction with $r < 0.01$. The minimal CHF is excluded; the non-minimal case remains viable but loses predictive simplicity.

\section{Summary of Spectral Results}
