# Horizon-Centered Cosmological Program

This repository contains a set of conceptual works exploring horizon-centered approaches in cosmology and gravitational physics.

The common theme across all documents is the structural role of:

- causal accessibility
- horizon-defined domains
- boundary-associated entropy

The project does not introduce new fundamental dynamical laws, modifications of General Relativity, or observational predictions. Instead, it investigates whether established cosmological and gravitational phenomena may admit alternative conceptual organization around causal structure and horizons.

The repository should be understood as a conceptual research program rather than as a completed physical theory.

---

# Scope

The program is:

- conceptual
- non-technical at its foundational level
- compatible with established phenomenology
- agnostic regarding microscopic mechanisms
- intentionally open-ended

The program is not:

- a replacement for ΛCDM
- a modification of General Relativity
- a quantum gravity theory
- a predictive cosmological model
- a unifying framework

No claims of observational discrepancy are made.

---

# Structural Organization

The documents are organized hierarchically.

## 1. Positioning

`positioning/On_the_Scope_and_Intent_of_Horizon_Centered_Approaches.pdf`

Primary entry point.

Defines:

- scope
- intended interpretation
- evaluation criteria
- limits of claims

This document should be read first.

---

## 2. Core Framework

`core/A_Causal–Horizon_Perspective_on_the_Early_Universe.pdf`

Introduces a horizon-centered interpretation of cosmology in which observable domains are treated as causally bounded structures.

The document operates primarily at an interpretative level and establishes structural consistency principles.

---

## 3. Ontological Layer

`ontology/Ontology_Of_Causally_Self-consistent_Regions.pdf`

Introduces causally self-consistent regions (CSCRs) as minimal ontological structures compatible with horizon-centered descriptions.

The document does not introduce a complete ontology.

---

## 4. Conceptual Scaffold

`scaffold/Causal_Horizons_As_A_Conceptual_Scaffold_In_Gravitational_And_Cosmological_Physics.pdf`

Identifies recurring structural principles shared across:

- horizon thermodynamics
- holographic viewpoints
- cosmological horizons
- causal-structure approaches

The scaffold is organizational and does not imply unification.

---

## 5. Effective Realization

`formalization/A_Horizon-Centered_Formalization_of_Cosmological_Dynamics.pdf`

Provides one possible effective realization of the conceptual framework.

Demonstrates that:

- standard Friedmann behavior may be reformulated through horizon thermodynamics
- perturbations may be represented through boundary variables

This realization does not exhaust the framework.

Alternative realizations remain possible.

---

# Reading Order

Recommended order:

1. Positioning note
2. Core framework
3. Ontology
4. Scaffold
5. Formal realization

---

# Evaluation Criteria

The program is intended to be evaluated according to:

- internal conceptual coherence
- structural consistency
- compatibility with established phenomenology
- interpretative usefulness

Absence of microscopic mechanisms or predictive structure should not be interpreted as inconsistency.

---

# Current Status

Work in progress.

Several questions remain intentionally open:

- microscopic horizon structure
- boundary-to-bulk mapping
- emergence of geometry
- effective realizations
- relation to quantum gravity

These open questions define future research directions.

---

# Use and Citation

The material may be read, discussed, shared, and cited for academic purposes.

If ideas developed here are used or extended, appropriate attribution is appreciated.