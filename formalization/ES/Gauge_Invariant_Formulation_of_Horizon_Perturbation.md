...
$$\delta t = -\frac{V}{a} \tag{19}$$

En este gauge, la perturbación métrica espacial es igual a $\mathcal{R}$:

$$\mathcal{R} = \Psi_\text{metric}^\text{com} = \Phi_\text{Newton} + H \cdot \frac{V}{a} \tag{20}$$

De la ligadura de momento (5): $H V/a = -(\dot{\Phi} + H\Phi)/(\varepsilon H^2)$.

Sustituyendo:

$$\mathcal{R} = \Phi - \frac{\dot{\Phi} + H\Phi}{\varepsilon H} = \Phi\left(\frac{1+\varepsilon}{\varepsilon}\right) - \frac{\dot{\Phi}}{\varepsilon H} \tag{21}$$
